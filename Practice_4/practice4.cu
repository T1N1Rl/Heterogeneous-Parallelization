...
__global__ void reductionGlobalOnlyKernel(const int* d_in, int* d_out, int n) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x; 
    if (idx < n) {
        // Каждый поток напрямую обращается к одной ячейке в Глобальной памяти.
        // Это создает огромную очередь и работает медленно.
...
void benchmark(int N) {
    ...
    int *d_out_global_only; // Указатель на массив результатов глобальной редукции
    ...
    cudaMalloc(&d_out_global_only, sizeof(int));
    cudaMemcpy(d_arr, h_arr.data(), N * sizeof(int), cudaMemcpyHostToDevice);  // Копирование исходного массива с CPU на GPU
    cudaMemset(d_out_global_only, 0, sizeof(int));

    auto start_global = high_resolution_clock::now();    // Засекаем старт
    reductionGlobalOnlyKernel<<<numBlocks, blockSize>>>(d_arr, d_out_global_only, N); // Запуск ядра
    cudaDeviceSynchronize();                        // Остановка CPU до полного завершения вычислений на GPU
    auto end_global = high_resolution_clock::now(); // Засекаем конец

    int h_res_global = 0;
    cudaMemcpy(&h_res_global, d_out_global_only, sizeof(int), cudaMemcpyDeviceToHost);
    
    cout << "Reduction (Global Only): " << duration<double, milli>(end_global - start_global).count() << " ms" << endl;
    cout << "Result Global: " << (h_res_global == cpu_sum ? "Correct" : "Error") << " (Sum: " << h_res_global << ")" << endl;
